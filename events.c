...
static unsigned int set_mask_ignore(unsigned int modifier)
{
	...
	if(Scr->IgnoreLockModifier) {
		modifier &= ~LockMask;
	}
	for(i = 0 ; i < 8 ; i++) {
		if(Scr->IgnoreModifier & ModifierMask [i]) {
			modifier &= ~ModifierMask [i];
		}
	}
	return modifier;
}
...
void HandleKeyPress(void)
{
	...
	if(ActiveMenu != NULL) {
		...
		else if(!strcmp(keynam, "Right") || !strcmp(keynam, "Return")) {
			item = ActiveItem;
		}
		else if(!strcmp(keynam, "Left") || !strcmp(keynam, "Escape")) {
			MenuRoot *menu;
