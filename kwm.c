...
addModule(WScreen *scr, Window window)
{
    ...
    for (ptr = scr->Focus; ptr!=NULL; ptr = ptr->prev) {
    ...
    /* send focused window */
    if (scr->Focus && scr->Focus->flags.focused) {
	sendClientMessage(scr, window, _XA_KWM_MODULE_WIN_ACTIVATE,
			  scr->Focus->client_win);
    }
...
#define SETSTR(hint, str) {\
	...
	XChangeProperty(dpy, scr->Root, a, XA_STRING, 8, PropModeReplace,\
...
wKWMGetWorkspaceName(WScreen *scr, int workspace)
{
    ...
    if (XGetWindowProperty(dpy, scr->Root, 
...
{
    setSimpleHint(scr->Root, _XA_KWM_RUNNING, 1);
}
...
wKWMShutdown(WScreen *scr, Bool closeModules)
{
    ...
    XDeleteProperty(dpy, scr->Root, _XA_KWM_RUNNING);
...
performCommand(WScreen *scr, char *command, XClientMessageEvent *event)
{
	...
	wHideOtherApplications(scr->Focus);
...
wKWMCheckRootHintChange(WScreen *scr, XPropertyEvent *event)
{
	...
	if (getSimpleHint(scr->Root, _XA_KWM_CURRENT_DESKTOP, &value)) {
#ifdef DEBUG1
	...
	if (getSimpleHint(scr->Root, _XA_KWM_NUMBER_OF_DESKTOPS, &value)) {
	...
	for (i = 0; i < MAX_WORKSPACES && i < scr->workspace_count; i++) {
		 ...
		 if (getAreaHint(scr->Root, event->atom, &area)) {
...
{
    setSimpleHint(scr->Root, _XA_KWM_CURRENT_DESKTOP,
...
wKWMUpdateActiveWindowHint(WScreen *scr)
{
    ...
    XChangeProperty(dpy, scr->Root, _XA_KWM_ACTIVE_WINDOW,
...
wKWMUpdateWorkspaceCountHint(WScreen *scr)
{
    ...
    setSimpleHint(scr->Root, _XA_KWM_NUMBER_OF_DESKTOPS,
		  scr->workspace_count);
...
wKWMUpdateWorkspaceNameHint(WScreen *scr, int workspace)
{
    ...
    XChangeProperty(dpy, scr->Root, _XA_KWM_DESKTOP_NAME_[workspace],
...
wKWMGetUsableArea(WScreen *scr, WArea *area)
{
    ...
    return getAreaHint(scr->Root, _XA_KWM_WINDOW_REGION_[0], area);
}
 ...
 */
    ...
    setAreaHint(scr->Root, _XA_KWM_WINDOW_REGION_[workspace],
		scr->totalUsableArea);
#endif
}
#endif /* not_used */
